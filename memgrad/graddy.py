...
def AdamTest(iterates: int = 1000):
    fig1 = prepPlot()
    a = adamtest.Adam(rosenbrock, grad_rosenbrock, x0.copy(), 0.3, 0.9, 0.99, 1e-8)
    obs, path, vh, mh = a.iterate(iterates)
    plt.plot(path[:, 0], path[:, 1], "r.-", label="calculated")
    plt.legend()
    plt.title(f"Adam, {iterates} iterations")
...
def ValleyTest(iterations=1000):
    ...
    plt.plot(restarted[:, 0], restarted[:, 1], "co", markersize=5, label="restarted")   
    plt.legend()
